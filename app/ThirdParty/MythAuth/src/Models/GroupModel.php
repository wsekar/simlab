...
class GroupModel extends Model
{
    ...
    public function inGroup($groupSlug, $userId)
    {
        // Ambil ID grup berdasarkan slug grup
        $group = $this->db->table('auth_groups')
                          ->select('id')
                          ->where('name', $groupSlug)
                          ->limit(1)
                          ->get()
                          ->getRow();

        // Jika grup ditemukan, lakukan pengecekan di tabel penghubung
        if ($group) {
            $groupId = $group->id;

            // Cek apakah pengguna masuk dalam grup berdasarkan ID grup dan ID pengguna
            $groupUser = $this->db->table('auth_groups_users')
                                  ->where('user_id', $userId)
                                  ->where('group_id', $groupId)
                                  ->countAllResults();
