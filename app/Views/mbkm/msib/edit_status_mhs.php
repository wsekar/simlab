                ...
                <div class="row my-4">
                    <div class="col">
                        <div class="card shadow mb-4">
                            ...
                            <div class="card-body">
                                <form method="POST"
                                    action="<?=base_url('mbkm/msib/update-status-mhs/' . $msib->id_mhs)?>">
                                    <input type="hidden" name="nama_instansi" value="<?= $msib->nama_instansi ?>">
                                    <input type="hidden" name="id_staf" value="<?= $msib->id_staf ?>">
                                    <input type="hidden" name="jenis_mbkm" value="<?= $msib->jenis_mbkm ?>">
                                    <input type="hidden" name="id_msib" value="<?=$msib->id_msib?>">
                                    <div class=" form-group mb-3">
                                        ...
                                        <select class="form-control select2" name="status_mahasiswa"
                                            ...
                                            <option value="pending"
                                                <?= $msib->status_mahasiswa == 'pending' ? 'selected' : ''?>>
                                                Pending
                                            </option>
                                            <option value="proses seleksi"
                                                <?= $msib->status_mahasiswa == 'proses seleksi' ? 'selected' : ''?>>
                                                Proses seleksi</option>
                                            <option value="lolos"
                                                <?= $msib->status_mahasiswa == 'lolos' ? 'selected' : ''?>>
                                                lolos</option>
                                            <option value="tidak lolos"
                                                <?= $msib->status_mahasiswa == 'tidak lolos' ? 'selected' : ''?>>
                                                tidak lolos</option>
                                            <option value="diambil"
                                                <?= $msib->status_mahasiswa == 'diambil' ? 'selected' : ''?>>
                                                diambil</option>
                                            <option value="tidak diambil"
                                                <?= $msib->status_mahasiswa == 'tidak diambil' ? 'selected' : ''?>>
