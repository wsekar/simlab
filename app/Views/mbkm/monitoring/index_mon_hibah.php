...
<main role="main" class="main-content">
    ...
    <div class="row my-3">
        ...
        <div class="col-md-12">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h2 class="page-title">Monitoring Kegiatan MBKM Hibah</h2>
                    <p class="card-text">
                        Data Mahasiswa Aktif Kegiatan MBKM Hibah
                    </p>
                </div>
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right text-sm">
                        ...
                        <li class="breadcrumb-item active"><small>Monitoring MBKM Hibah</small></li>
                    </ol>
                </div>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td> <a class="btn btn-outline-primary" type="button"
                                            href="<?= base_url('mbkm/monitoring/detail-mbkm-hibah/' . $a->id_mbkm_fix) ?>">Monitoring</a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                <?php 
...
$(document).ready(function() {
    ...
    $('#prosesFilter').on('click', function(event) {
        ...
        if (th_masuk != '') {
            ...
            var url = "<?php echo base_url('mbkm/monitoring/filter-adm-hibah'); ?>" +
                "/" +
                th_masuk;
            <?php elseif (has_permission('dosen')): ?>
            var url = "<?php echo base_url('mbkm/monitoring/filter-dsn-hibah'); ?>" + "/" +
                th_masuk;
            <?php endif;?>
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data, function(i, item) {
                    var url_detail =
                        '<?=base_url('mbkm/monitoring/detail-mbkm-hibah');?>' +
                        '/' + item.id_mbkm_fix;
                    $('#dataTable-1 tbody')
                    <?php if (has_permission('admin') || has_permission('koor-mbkm')): ?>
                        .append('<tr><td>' + (i++ + 1) +
    ...
    $(document).ready(function() {
        $('#resetFilter').click(function() {
            ...
            $.ajax({
                url: '<?php echo base_url('mbkm/monitoring/mbkm-hibah'); ?>',
                method: "GET",
                dataType: 'json',
                success: function(data) {
                    // Tampilkan data ke dalam tabel
